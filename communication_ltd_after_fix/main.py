        ...
        safe_clients.append(
            {
                ...
                "sector": html.escape(client.sector),
                "created_at": client.created_at
            }
        )
    return safe_clients

@app.post("/login-protected")
def secure_login(data: schemas.UserLogin):
    ...
    if utils.verify_password(data.password, stored_hashed_password):
        return {"message": f"Login successful! Welcome {user[1]}"}
    else:
        raise HTTPException(status_code=401, detail="Invalid credentials")
